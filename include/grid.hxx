...
#ifndef LP_MP_GRID_HXX
#define LP_MP_GRID_HXX
...
namespace LP_MP {
    ...
    struct grid {
        ...
        std::array<std::size_t,2> node_to_grid(const std::size_t i) const 
        {
            ...
            const std::size_t x = i/size_y;
            const std::size_t y = i%size_y;
            return {x,y}; 
        }
        ...
        std::vector<std::size_t> chain(const std::size_t chain_no) const
        {
            ...
            if(size_x == 1 || size_y == 1) {
                ...
                if(chain_no < size_x) { return horizontal_chain(chain_no); }
                else { return vertical_chain(chain_no - size_x); }
            }
        }

        std::vector<std::size_t> horizontal_chain(const std::size_t i) const 
        {
            assert(i < size_x);
            std::vector<std::size_t> nodes;
            nodes.reserve(size_y);
            for(std::size_t j=0; j<size_y; j++) {
                nodes.push_back(i*size_y + j);
            }
            return nodes;
        }

        std::vector<std::size_t> vertical_chain(const std::size_t j) const 
        {
            assert(j < size_y);
            std::vector<std::size_t> nodes;
