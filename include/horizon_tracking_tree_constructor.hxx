#ifndef LP_MP_HORIZON_TRACKING_TREE_CONSTRUCTOR_HXX
#define LP_MP_HORIZON_TRACKING_TREE_CONSTRUCTOR_HXX
...
max_tree_factor_container* add_max_tree(const std::vector<std::array<INDEX,2>>& messagePassingSchedule,
                                    const three_dimensional_variable_array<REAL>& maxPairwisePotentials,
                                    std::vector<INDEX>& numLabels, factor_tree<FMC>* t = nullptr)
{
    ...
    for(const auto& currentEdge : messagePassingSchedule) {
        INDEX i = fmin(currentEdge[0], currentEdge[1]);
        INDEX j = fmax(currentEdge[0], currentEdge[1]);
...
void construct_horizon_tracking_problem_on_grid_to_trees(const horizon_tracking_input& input, SOLVER& solver, HORIZON_TRACKING_CONSTRUCTOR& tree_constructor)
{
    ...
    INDEX numNodes = input.mrf.no_variables();
    std::vector<INDEX> numLabels(numNodes);
    // we assume that bottleneck potential and mrf potential have same number of variables and in same order. 
    for (INDEX n = 0; n < numNodes; n++) { numLabels[n] = input.mrf.cardinality(n); }
    ...
    for(const auto& bottleneck_potential : input.bottleneck_potentials) {
        ...
        std::vector<three_dimensional_variable_array<REAL>> max_pairwise_potentials_on_trees(sp_trees.number_trees());
        ...
        for(std::size_t t=0; t<sp_trees.number_trees(); ++t) {
            ...
            for (const auto& current_pairwise : current_tree_schedule) {
                INDEX pairwise_index = sp_trees.get_pairwise_index(current_pairwise[0], current_pairwise[1]);
...
#endif // LP_MP_HORIZON_TRACKING_TREE_CONSTRUCTOR_HXX
